...
async function traerUsuarios () {
  const response = await fetch('traerEstudiantes.php', {
    method: 'GET'
  })
...
function filaNueva (infoPersona, pos) {
  const fila = `<tr id="${pos}">
                  <td id='txtId${pos}' class='pl-4'>${infoPersona.IDEstudiante}</td>
                  ...
                  <td id='txtTeorico${pos}'>${infoPersona.teorico}</td>
...
function guardarCambios (pos) {
  $.ajax({
    url: 'modificar-estudiante.php',
    method: 'POST',
    data: {
      IDEstudiante: datosUsuarios[pos].IDEstudiante,
  ...
  $('#txtTeorico' + pos).html($('#txtTeorico').val())
  ...
  datosUsuarios[pos].teorico = $('#txtTeorico').val()
...
function mostrarModal (pos) {
  ...
  const teoricoValue = datosUsuarios[pos].teorico
  $('#txtTeorico option').each(function() {
    if ($(this).val() === teoricoValue) {
  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      const pos = datosUsuarios.findIndex(
        estudiante => estudiante.documento === documento
      )
      if (pos !== -1) {
        $('#' + pos).remove()
...
function filtrarEstudiantes () {
  ...
  let count = 0
  datosUsuarios.forEach((estudiante, index) => {
    ...
    if (
      ...
      teorico.toLowerCase().includes(searchTerm) ||
      nombreCompleto.includes(searchTerm)
    ) {
      const highlightedEstudiante = {
        ...estudiante,
        ...
        teorico: highlightMatch(teorico, searchTerm),
        email: highlightMatch(email, searchTerm),
      }

      filaNueva(highlightedEstudiante, index)
      count++
    }
  })
}
...
$('#searchInput').on('input', filtrarEstudiantes)
$('#limitSelect').on('change', filtrarEstudiantes)

$(document).ready(filtrarEstudiantes)
...
function renderTable (data) {
  ...
  paginatedData.forEach((estudiante, index) => {
    filaNueva(estudiante, start + index)
  })
